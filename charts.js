...
function showError(msg) {
  ...
  <div style="background:#2c2c2c;color:#ffb3b3;border:1px solid #ff6b6b;padding:1.5rem;margin:2rem;border-radius:10px;text-align:center;">
  ${msg}</div>`;
}
...
function pearson(x,y){const n=Math.min(x.length,y.length);const mx=mean(x),my=mean(y);const num=x.reduce((a,_,i)=>a+(x[i]-mx)*(y[i]-my),0);const den=Math.sqrt(x.reduce((a,v)=>a+(v-mx)**2,0)*y.reduce((a,v)=>a+(v-my)**2,0));return den===0?0:num/den;}
function mean(a){return a.reduce((x,y)=>x+y,0)/a.length;}
function renderCorrTable(matrix,cols){let html=`<table class='corr-table'><tr><th></th>${cols.map(c=>`<th>${c}</th>`).join("")}</tr>`;for(let i=0;i<cols.length;i++){html+=`<tr><th>${cols[i]}</th>`;for(let j=0;j<cols.length;j++){const v=matrix[i][j].toFixed(2);html+=`<td style='background-color:${corrColor(v)}'>${v}</td>`;}html+="</tr>";}html+="</table>";return html;}
function corrColor(v){const n=parseFloat(v);const r=n>0?40:200;const g=n>0?200:60;const alpha=Math.abs(n)*0.8+0.2;return`rgba(${r},${g},100,${alpha})`;}
